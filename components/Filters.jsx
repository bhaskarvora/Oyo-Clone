...
//         <input type="range" name="price" id="price" min={500} max={3500} onChange={(e) => setPrice(e.target.value)}
  ...
  return (
    <>
      <div className="border-2 border-red-500 rounded-md m-5 h-auto py-10 px-3 ">
        ...
        <div>
          <button className="w-40 h-10 bg-green-300 cursor-pointer my-3" onClick={handlePrice}>
            Search
          </button>
        </div>
        ...
        <div className="my-10">
          ...
          {list?.map((e) => {
            return (
              <p key={e} className="grid grid-cols-4 my-3">
                <label htmlFor={`checkbox-${e}`} className="col-span-2">{e}</label>
                <input
                  type="checkbox"
                  name="checkbox"
                  id={`checkbox-${e}`}
